...
export default async function sendAlertToSMSViaAWS(product_url, title, store) {
	if (SMS_METHOD == "Amazon Web Services") {
		if (!fs.existsSync(".env")) {
			console.error(
				moment().format("LTS") + ": Error sending AWS SMS alert, .env file not present."
			);
		} else if (
			service.region == "" ||
			service.key == "" ||
			...
			console.error(
				moment().format("LTS") +
					": Error sending AWS SMS alert, ensure AWS information is filled."
			);
		} else {
			...
			sns.publish(parameters, (error) => {
				if (error) {
					console.error(
						moment().format("LTS") + ": Error sending email alert",
						error,
						error.stack
					);
				}
