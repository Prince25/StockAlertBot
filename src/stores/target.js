...
export default async function target(html) {
	try {
		...
		let jsonResponse = await fetchPage(
			'https://api.target.com/shipt_deliveries/v1/stores?zip=' + TARGET_ZIP_CODE +
			'&key=' + TARGET_KEY,
			'target',
			true,
			new Set(),
		...
		const location_id = jsonResponse ? jsonResponse?.closest_eligible_store?.location_id : undefined
		...
		if (jsonResponse && jsonResponse.store_options && jsonResponse.store_options.length > 0)
			in_store = jsonResponse.store_options.some(
				store => {
