...
export default async function newegg(url, interval) {
    if (url.includes('ComboDealDetails')) neweggCombos(url, interval)
    else {
        try {
...
async function neweggCombos(url, interval) {
    try {
        let res = await axios.get(url, {
            headers: {
                'User-Agent': USER_AGENTS[Math.floor(Math.random() * USER_AGENTS.length)]
        ...
        if (res && res.status == 200) {
            ...
            let inventory = doc.getElementsByClassName('atnPrimary')
            let image = 'https:' + doc.getElementById('mainSlide_0').getAttribute('src')
